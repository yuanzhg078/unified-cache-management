...
- `kv_semantics/metrics/src/metrics.cc`：`gBackend`、`gBackendFast`、`LoadBackendFast()`、`Initialize()`、`Shutdown()` 和 facade 打点入口。
...
- `kv_semantics/metrics/src/standalone_metrics_backend.cc`：`DeltaSlot`、`ThreadBuffer`、`WriteGuard`、`BeginWrite()`、`EndWrite()`、`SwitchWriteSlot()`、`WaitUntilInactive()` 和 `Aggregate()`。
...
- `kv_semantics/metrics/src/standalone_metrics_backend.cc`：`ThreadLocalBufferCache`、`NextCollectorGeneration()`、`generation_`、`GetThreadBuffer()` 和 `buffers_`。
...
- `kv_semantics/tests/metrics/standalone_metrics_test.cc`：`DoesNotLoseUpdatesDuringConcurrentFlush`。
