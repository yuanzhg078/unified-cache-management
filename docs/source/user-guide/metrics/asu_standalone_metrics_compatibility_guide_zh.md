...
metrics.drain_interval_ms=1000
...
当前落地代码做了一项简化：standalone registry 在 `Add/Set/Observe` 时直接维护 Prometheus 累计状态，HTTP 请求只序列化只读快照，因此不需要调用 UCM 的 `GetAllStatsAndClear()`，也不需要额外 dispatcher/drain 线程。Dispatcher 仍适用于需要复用“增量 buffer + destructive drain”采集模型的后续版本。
...
- 内置 Linux HTTP server，提供 `/metrics` 和 `/healthz`。
