...
`kv-test` 使用 upstream 的通用 `trans` 运行时管理 payload buffer；ASU client、transport 和 kv-test 显式链接同一个 `asu_metrics` facade。只有启用 UCM adapter 时才会额外链接 `libucm_metrics.so`。
...
此时可用 fake provider 在 simu 环境构建；`kv-test`、`libasu_client.so` 和 `libasu_transport.so` 只依赖 `libasu_metrics.so`，不依赖 `libucm_metrics.so`。真实 Ascend provider 构建仍使用 `-DRUNTIME_ENVIRONMENT=ascend`。
